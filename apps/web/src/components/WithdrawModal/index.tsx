"use client";

import { getPublicApiUrl } from "@/lib/backend-url";
import { useEffect, useMemo, useState } from "react";

type WithdrawModalProps = {
  userId: string;
  /** The user's managed (vault) balance in USDC. */
  vaultBalanceAmount: number | null;
  onClose: () => void;
  onSuccess?: () => void;
};

export function WithdrawModal({
  userId,
  vaultBalanceAmount,
  onClose,
  onSuccess,
}: WithdrawModalProps) {
  const [visible, setVisible] = useState(false);
  const [amount, setAmount] = useState("");
  const [isMax, setIsMax] = useState(false);
  const [status, setStatus] = useState<
    "idle" | "sending" | "success" | "error"
  >("idle");
  const [error, setError] = useState("");

  useEffect(() => {
    const t = requestAnimationFrame(() => setVisible(true));
    return () => cancelAnimationFrame(t);
  }, []);

  useEffect(() => {
    document.body.style.overflow = "hidden";
    return () => {
      document.body.style.overflow = "";
    };
  }, []);

  const handleClose = () => {
    if (status === "sending") return;
    setVisible(false);
    setTimeout(onClose, 220);
  };

  const parsedAmount = useMemo(() => parseFloat(amount), [amount]);
  const hasValidAmount = Boolean(
    vaultBalanceAmount !== null &&
    !Number.isNaN(parsedAmount) &&
    parsedAmount > 0 &&
    parsedAmount <= vaultBalanceAmount + 1e-9,
  );

  const setMax = () => {
    if (vaultBalanceAmount === null) return;
    setAmount(vaultBalanceAmount.toFixed(2));
    setIsMax(true);
    if (error) setError("");
  };

  const handleWithdraw = async () => {
    if (!hasValidAmount) return;
    setError("");
    setStatus("sending");

    try {
      const res = await fetch(getPublicApiUrl("/api/withdraw"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(
          isMax ? { userId, max: true } : { userId, amount: parsedAmount },
        ),
      });
      const json = await res.json();

      if (!res.ok) {
        setError(json.message ?? json.error ?? "Withdraw failed. Try again.");
        setStatus("error");
        return;
      }

      setStatus("success");
      onSuccess?.();
      setTimeout(handleClose, 1400);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Withdraw failed. Try again.",
      );
      setStatus("error");
    }
  };

  const busy = status === "sending";

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center px-6 touch-none"
      style={{
        touchAction: "none",
        backgroundColor: `rgba(0,0,0,${visible ? 0.7 : 0})`,
        transition: "background-color 220ms ease",
      }}
      onClick={handleClose}
    >
      <div
        className="w-full max-w-sm bg-surface px-5 pt-5 pb-6 flex flex-col gap-5 rounded-2xl"
        style={{
          opacity: visible ? 1 : 0,
          transform: visible
            ? "scale(1) translateY(0)"
            : "scale(0.95) translateY(12px)",
          transition: "opacity 220ms ease, transform 220ms ease",
        }}
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between">
          <span className="font-headline text-[10px] uppercase tracking-widest text-white/40 font-bold">
            Withdraw from Vault
          </span>
          <button
            onClick={handleClose}
            className="w-7 h-7 flex items-center justify-center text-white/40 active:text-white"
            aria-label="Close"
            disabled={busy}
          >
            <span className="material-symbols-outlined text-lg">close</span>
          </button>
        </div>

        {/* Managed balance */}
        <div className="flex items-center justify-between rounded-xl bg-background px-4 py-3">
          <p className="text-[10px] uppercase tracking-widest text-white/40">
            In vault
          </p>
          <p className="font-headline text-lg font-bold text-white">
            ${vaultBalanceAmount?.toFixed(2) ?? "0.00"}
          </p>
        </div>

        {/* Amount */}
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            <p className="font-headline text-[10px] uppercase tracking-widest text-white/40 font-bold">
              Withdraw Amount
            </p>
            <button
              type="button"
              onClick={setMax}
              className="text-[10px] uppercase tracking-widest font-bold text-accent active:scale-95 transition-transform"
            >
              Max
            </button>
          </div>
          <div className="bg-background flex items-center px-4 py-3">
            <span className="text-white/30 font-bold mr-1 select-none">$</span>
            <input
              type="number"
              inputMode="decimal"
              value={amount}
              onChange={(e) => {
                setAmount(e.target.value);
                setIsMax(false);
                if (status !== "idle") setStatus("idle");
                if (error) setError("");
              }}
              placeholder="0.00"
              className="flex-1 bg-transparent outline-none text-white placeholder:text-white/20 appearance-none"
              style={{ fontSize: "16px" }}
            />
            <span className="text-white/30 text-xs uppercase tracking-wider ml-2">
              USDC
            </span>
          </div>
        </div>

        <p className="text-[11px] text-white/45 leading-relaxed">
          Genie returns this from your vault to your wallet instantly — no
          signature, no popup.
        </p>

        {/* Status */}
        {status === "success" && (
          <p className="text-xs text-accent text-center font-headline font-bold uppercase tracking-widest">
            Withdrawn to your wallet!
          </p>
        )}
        {status === "error" && error && (
          <p className="text-xs text-red-400 text-center leading-relaxed">
            {error}
          </p>
        )}
        {!hasValidAmount && amount.trim().length > 0 && status !== "error" && (
          <p className="text-[11px] text-red-300 text-center">
            Enter an amount above 0 and no more than your vault balance.
          </p>
        )}

        {/* CTA */}
        <button
          onClick={handleWithdraw}
          disabled={!hasValidAmount || busy}
          className="w-full bg-accent text-black font-headline font-bold text-sm uppercase tracking-widest py-4 active:scale-95 transition-transform disabled:opacity-30 disabled:pointer-events-none rounded-lg"
        >
          {busy
            ? "Withdrawing…"
            : `Withdraw${hasValidAmount ? ` $${parsedAmount.toFixed(2)}` : ""}`}
        </button>
      </div>
    </div>
  );
}
